...
const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      if (username === 'junaid' && password === '199912') {
        onLogin();
      } else {
    ...
    <div className="min-h-screen bg-obsidian-bg flex items-center justify-center p-4 relative overflow-hidden">
      ...
      >
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-obsidian-textMuted text-sm font-mono">Restricted to authorized personnel only.</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-xs font-mono text-neon-cyan mb-2 uppercase tracking-wider">Identity</label>
            <input 
              type="text" 
              value={username}
          ...
          <button 
            type="submit" 
            disabled={isLoading}
            className="w-full bg-neon-purple text-white font-bold py-3 rounded-lg hover:bg-neon-purple/90 transition-all flex items-center justify-center gap-2 mt-6 disabled:opacity-70"
          >
            {isLoading ? (
        ...
        <div className="mt-8 pt-6 border-t border-obsidian-border text-center">
           <div className="flex items-center justify-center gap-2 text-xs text-obsidian-textMuted font-mono">
             <Lock size={12} />
